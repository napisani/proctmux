...
test "literal block scalar as map value" {
    const source =
        \\docs: |
        \\  line one
        \\  line two
    ...
    const map_data = tree.extraData(Map, tree.nodeData(doc_value.unwrap().?).extra);
    try testing.expectEqual(2, map_data.data.map_len);

    var entry_data = tree.extraData(Map.Entry, map_data.end);
    try expectStringValueMapEntry(tree, entry_data.data, "docs", "line one\nline two\n");

    entry_data = tree.extraData(Map.Entry, entry_data.end);
    try expectValueMapEntry(tree, entry_data.data, "next", "value");
}
