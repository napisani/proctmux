...
Each process runs inside a pseudo-terminal (PTY) allocated with `forkpty` in
`src/proc/pty.zig`. The PTY provides a master/slave pair:
...
The default PTY size is **80 columns x 24 rows**. This can be overridden per-process via `terminal_cols` and `terminal_rows` in the process config. The size is passed to `forkpty` as a `winsize`, and the child can query it with TIOCGWINSZ.

**Output pipeline:**

```
Child process -> PTY slave -> PTY master -> output capture thread -> RingBuffer (1MB)
```

Each process has a dedicated 1MB ring buffer (`src/ring/root.zig`) that stores scrollback output. The ring buffer is circular -- when it fills up, the oldest data is silently overwritten. The output capture thread in `src/proc/output.zig` runs for the lifetime of the PTY and forwards all output from the master fd to the ring buffer.

**Process IDs:** Each process gets a unique sequential integer ID starting at 1, assigned while building `AppState` from sorted config key order (`src/domain/state.zig`).
...
The `shell` and `cmd` fields are mutually exclusive and resolve differently (`src/proc/builder.zig`):
...
- **`cmd`**: Executed directly with no shell wrapper. The first element is the executable; the rest are arguments.

If neither is specified, the process fails to start with "no shell or cmd specified".

### Environment

The child process inherits the full environment of the proctmux parent process, with two layers of customization (`src/proc/env.zig`):
...
Stopping follows a signal escalation sequence (`src/proc/controller.zig`):
...
Key behavior (`src/proc/on_kill.zig`):
...
- Has a **30-second timeout**. If the hook does not finish in time, it is killed and the hook reports failure.
- Runs from the process release path, so a given process instance executes the hook at most once.
...
The restart sequence (`src/primary/root.zig`):
...
Process status is defined in `src/domain/process.zig`:
...
Processes with `autostart: true` are started during primary server startup (`src/primary/root.zig`):
...
When the TUI client quits (press `q` or `ctrl+c`), it sends a `stop-running` command to the primary server (`src/tui/client_model.zig`). The primary server then stops **all** currently running processes in parallel, using the full signal escalation sequence for each one (`src/primary/root.zig`).

This means quitting the TUI stops everything. Processes are not intentionally left running in the background. If one process fails to stop cleanly, the failure is logged and shutdown continues for the remaining processes.
...
Each process has a 1MB circular ring buffer (`src/ring/root.zig`) that stores its output history.

When the viewer switches to a process, it uses `snapshotAndSubscribe()` (`src/ring/root.zig`, called from `src/viewer/root.zig`) to atomically:
...
2. Relays pending live output from the subscribed reader to stdout.

When switching away from a process, the viewer removes the reader from the ring buffer and repeats the process for the newly selected process.
