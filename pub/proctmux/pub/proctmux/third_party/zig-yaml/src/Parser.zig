...
fn leafValue(self: *Parser, gpa: Allocator) ParseError!Node.OptionalIndex {
    ...
    while (self.token_it.next()) |tok| {
        switch (tok.id) {
            ...
            .literal => {
                const literal_token: Token.Index = @enumFromInt(@intFromEnum(self.token_it.pos) - 1);
                const raw = self.rawString(literal_token, literal_token);
                if (raw.len > 0 and raw[0] == '|') {
                    return self.blockLiteralValue(gpa, node_index, node_start, literal_token, raw);
                }
            },
            .space => {
                ...
                if (self.token_it.peek()) |peek| {
                    switch (peek.id) {
                        .new_line, .comment, .doc_start, .doc_end, .flow_map_end, .flow_seq_end, .comma, .eof => {
...
fn blockLiteralValue(
    self: *Parser,
    gpa: Allocator,
    node_index: Node.Index,
    node_start: Token.Index,
    literal_token: Token.Index,
    indicator: []const u8,
) ParseError!Node.OptionalIndex {
    const literal = self.tokens.items(.token)[@intFromEnum(literal_token)];
    var cursor = literal.loc.end;

    while (cursor < self.source.len and (self.source[cursor] == ' ' or self.source[cursor] == '\t')) {
        cursor += 1;
    }
    if (cursor < self.source.len and self.source[cursor] == '#') {
        while (cursor < self.source.len and self.source[cursor] != '\n' and self.source[cursor] != '\r') {
            cursor += 1;
        }
    }
    if (cursor < self.source.len and self.source[cursor] == '\r') cursor += 1;
    if (cursor >= self.source.len or self.source[cursor] != '\n') return error.MalformedYaml;
    cursor += 1;
    ...
    while (scan < self.source.len) {
        const line_start = scan;
        var line_end = scan;
        while (line_end < self.source.len and self.source[line_end] != '\n' and self.source[line_end] != '\r') {
            line_end += 1;
        }

        var next_line = line_end;
        if (next_line < self.source.len and self.source[next_line] == '\r') next_line += 1;
        const had_newline = next_line < self.source.len and self.source[next_line] == '\n';
        if (had_newline) next_line += 1;

        var indent: usize = 0;
        while (line_start + indent < line_end and self.source[line_start + indent] == ' ') {
            indent += 1;
        }

        const is_blank = line_start + indent >= line_end;
        if (!is_blank) {
            if (content_indent == null) content_indent = indent;
            if (indent < content_indent.?) break;
            const slice_start = @min(line_start + content_indent.?, line_end);
            try content.appendSlice(self.source[slice_start..line_end]);
        }

        if (content_indent != null and had_newline) try content.append('\n');
        scan = next_line;
    }

    if (indicator.len >= 2 and indicator[1] == '-' and content.items.len > 0 and content.items[content.items.len - 1] == '\n') {
        _ = content.pop();
    }

    const string = try self.addString(gpa, content.items);
    const next_token = self.tokenIndexAtOrAfter(scan);
    self.token_it.seekTo(next_token);
    const end_token: Token.Index = if (@intFromEnum(next_token) > 0)
        @enumFromInt(@intFromEnum(next_token) - 1)
    else
        literal_token;
...
fn tokenIndexAtOrAfter(self: *Parser, offset: usize) Token.Index {
    var index = @intFromEnum(self.token_it.pos);
    const tokens = self.tokens.items(.token);
    while (index < tokens.len and tokens[index].loc.start < offset) {
        index += 1;
    }
