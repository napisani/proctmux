...
fn dupeMapKey(gpa: Allocator, tree: Tree, key_index: Token.Index) YamlError![]u8 {
    const token = tree.token(key_index);
    const raw = tree.rawString(key_index, key_index);

    return switch (token.id) {
        .single_quoted => dupeSingleQuoted(gpa, raw),
        .double_quoted => dupeDoubleQuoted(gpa, raw),
        else => gpa.dupe(u8, raw),
    };
}

fn dupeSingleQuoted(gpa: Allocator, raw: []const u8) YamlError![]u8 {
    assert(raw[0] == '\'' and raw[raw.len - 1] == '\'');
    const raw_no_quotes = raw[1 .. raw.len - 1];

    var out: std.ArrayListUnmanaged(u8) = .empty;
    errdefer out.deinit(gpa);
    try out.ensureTotalCapacity(gpa, raw_no_quotes.len);

    var index: usize = 0;
    while (index < raw_no_quotes.len) : (index += 1) {
        const c = raw_no_quotes[index];
        if (c == '\'') {
            if (index + 1 >= raw_no_quotes.len or raw_no_quotes[index + 1] != '\'') {
...
fn dupeDoubleQuoted(gpa: Allocator, raw: []const u8) YamlError![]u8 {
    assert(raw[0] == '"' and raw[raw.len - 1] == '"');
    const raw_no_quotes = raw[1 .. raw.len - 1];
    ...
    while (index < raw_no_quotes.len) : (index += 1) {
        ...
        index += 1;
        if (index >= raw_no_quotes.len) return error.InvalidEscapeSequence;

        switch (raw_no_quotes[index]) {
    ...
    fn fromNode(gpa: Allocator, tree: Tree, node_index: Node.Index) YamlError!Value {
        ...
        switch (tag) {
            ...
            .map_single => {
                ...
                const key = try dupeMapKey(gpa, tree, entry.key);
            ...
            .map_many => {
                ...
                for (0..map.data.map_len) |_| {
                    ...
                    const key = try dupeMapKey(gpa, tree, entry.data.key);
                    errdefer gpa.free(key);
