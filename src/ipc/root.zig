...
test "command request serializes as Go-compatible JSON line" {
    const line = try protocol.commandRequestLine(std.testing.allocator, "42", .start, "web");
...
test "command request omits empty label like Go omitempty" {
    const line = try protocol.commandRequestLine(std.testing.allocator, "7", .list, "");
    defer std.testing.allocator.free(line);
...
test "command request escapes JSON string fields like Go" {
    const line = try protocol.commandRequestLine(std.testing.allocator, "44", .start, "web \"quoted\" \\ worker");
    defer std.testing.allocator.free(line);
...
test "response line serialization matches Go omitempty shape" {
    ...
    const line = try protocol.responseLine(std.testing.allocator, response);
    defer std.testing.allocator.free(line);
...
test "state message serializes redacted state and process views" {
    ...
    var fake_controller = FakeProcessController{ .status = .running, .pid = 12345 };
    const line = try protocol.stateLine(
        std.testing.allocator,
        &app_state,
...
test "state message parser round-trips redacted state and process views" {
    ...
    var update = try protocol.parseStateLine(std.testing.allocator, line);
    defer update.deinit();

    try std.testing.expectEqual(@as(u32, 1), update.state.current_proc_id);
    ...
    try std.testing.expectEqual(@as(u32, 1), update.process_views[0].id);
...
test "persistent client consumes state updates and command responses" {
    ...
    defer cfg.deinit();
    try config.defaults.apply(&cfg, std.testing.allocator);
    try putTestProcess(&cfg, "api", "npm run dev");
    ...
    var fake = FakeCommandHandler{};
    var state_provider = FakeStateProvider{ .line = state_line };
    var stopped = std.atomic.Value(bool).init(false);
    const thread = try std.Thread.spawn(.{}, server.serveCommandsAtPathWithState, .{
        std.testing.allocator,
        path,
        FakeCommandHandler.handler(&fake),
    ...
    try std.testing.expectEqual(@as(u32, 1), initial.state.current_proc_id);
...
test "command server rejects unauthorized peer before handling request" {
    ...
    var fake = FakeCommandHandler{};
    var authorizer = FakePeerAuthorizer{ .err = error.UnauthorizedPeer };
    var capture = ServerErrorCapture{};
...
test "command server authorizes peer before serving request" {
    ...
    var fake = FakeCommandHandler{};
    var authorizer = FakePeerAuthorizer{};
    var capture = ServerErrorCapture{};
    const thread = try std.Thread.spawn(.{}, runOneShotServerWithAuthorizer, .{
        path,
        &fake,
...
test "command server sends initial state before reading client commands" {
    ...
    var fake = FakeCommandHandler{};
    var state_provider = FakeStateProvider{};
    var stopped = std.atomic.Value(bool).init(false);
    const thread = try std.Thread.spawn(.{}, server.serveCommandsAtPathWithState, .{
        std.testing.allocator,
        path,
    ...
    const initial_line = try readTestLine(std.testing.allocator, stream);
    ...
    const response_line = try readTestLine(std.testing.allocator, stream);
...
test "state command server keeps client open and broadcasts mutations" {
    ...
    const broadcast_line = try readTestLine(std.testing.allocator, stream);
    defer std.testing.allocator.free(broadcast_line);
    try std.testing.expectEqualStrings(state_provider.line, broadcast_line);

    const start_response_line = try readTestLine(std.testing.allocator, stream);
    ...
    const second_broadcast_line = try readTestLine(std.testing.allocator, stream);
    ...
    const stop_response_line = try readTestLine(std.testing.allocator, stream);
...
test "state command server broadcasts mutations to other connected clients" {
    ...
    const first_initial = try readTestLineTimeout(std.testing.allocator, first, 200);
    defer std.testing.allocator.free(first_initial);
    try std.testing.expectEqualStrings(state_provider.line, first_initial);

    const second_initial = try readTestLineTimeout(std.testing.allocator, second, 200);
...
const FakeProcessController = struct {
    ...
    fn getProcessStatus(context: *anyopaque, _: u32) domain.process.ProcessStatus {
...
const FakeCommandHandler = struct {
    ...
    fn handler(self: *FakeCommandHandler) server.CommandHandler {
        return .{
            .context = self,
            .handle = handle,
        };
    ...
    fn handle(
        ...
        process_list[0] = .{
            .name = try allocator.dupe(u8, request.label),
            .running = true,
            .index = 1,
        };
        errdefer process_list[0].deinit(allocator);
...
fn putTestProcess(cfg: *config.schema.Config, label: []const u8, shell: []const u8) !void {
    var proc_cfg = config.schema.ProcessConfig.empty(std.testing.allocator);
    errdefer proc_cfg.deinit(std.testing.allocator);
...
const FakeStateProvider = struct {
    ...
    fn provider(self: *FakeStateProvider) server.StateProvider {
        return .{
            .context = self,
...
const ServerErrorCapture = struct {
    err: ?anyerror = null,
};

const FakePeerAuthorizer = struct {
    err: ?anyerror = null,
    called: bool = false,
    fd: std.posix.fd_t = -1,

    fn authorizer(self: *FakePeerAuthorizer) server.PeerAuthorizer {
        return .{
            .context = self,
            .authorize = authorize,
        };
...
fn readTestLine(allocator: std.mem.Allocator, stream: std.net.Stream) ![]const u8 {
...
fn readTestLineTimeout(allocator: std.mem.Allocator, stream: std.net.Stream, timeout_ms: i32) ![]const u8 {
    var out = std.array_list.Managed(u8).init(allocator);
    errdefer out.deinit();
