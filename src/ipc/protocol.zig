...
pub fn parseCommandRequestLine(allocator: std.mem.Allocator, line: []const u8) !CommandRequest {
...
pub fn responseLine(allocator: std.mem.Allocator, response: Response) ![]const u8 {
    var buf = std.array_list.Managed(u8).init(allocator);
    errdefer buf.deinit();
...
pub fn parseResponseLine(allocator: std.mem.Allocator, line: []const u8) !Response {
    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, line, .{});
    defer parsed.deinit();
...
pub fn parseStateLine(allocator: std.mem.Allocator, line: []const u8) !StateUpdate {
    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, line, .{});
    defer parsed.deinit();
    ...
    var app_state = domain.state.AppState{
        ...
        .current_proc_id = try getOptionalU32(state_obj, "CurrentProcID", 0),
...
fn appendAppState(buf: *std.array_list.Managed(u8), state: *const domain.state.AppState) !void {
    ...
    try buf.writer().print(",\"CurrentProcID\":{},\"Processes\":[", .{state.current_proc_id});
...
fn appendProcess(buf: *std.array_list.Managed(u8), process: domain.process.Process) !void {
    try buf.writer().print("{{\"ID\":{},\"Label\":", .{process.id});
...
fn appendProcessView(buf: *std.array_list.Managed(u8), view: domain.process.ProcessView) !void {
    try buf.writer().print("{{\"ID\":{},\"Label\":", .{view.id});
...
fn parseProcessListItem(allocator: std.mem.Allocator, value: std.json.Value) !ProcessListItem {
